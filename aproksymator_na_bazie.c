...
double fi(double a, double b, int n, int i, double x) {
  ...
  if ((x < hx[0]) || (x > hx[4]))
    ...
    return 1 / h3 *
           (h3 + 3 * h * h * (x - hx[1]) + 3 * h * (x - hx[1]) * (x - hx[1]) -
            3 * (x - hx[1]) * (x - hx[1]) * (x - hx[1]));
  else if (x > hx[2] && x <= hx[3])
    return 1 / h3 *
           (h3 + 3 * h * h * (hx[3] - x) + 3 * h * (hx[3] - x) * (hx[3] - x) -
            3 * (hx[3] - x) * (hx[3] - x) * (hx[3] - x));
...
double dfi(double a, double b, int n, int i, double x) {
  ...
  if ((x < hx[0]) || (x > hx[4]))
    ...
    return 1 / h3 *
           (3 * h * h + 6 * h * (x - hx[1]) - 9 * (x - hx[1]) * (x - hx[1]));
  else if (x > hx[2] && x <= hx[3])
    return 1 / h3 *
           (-3 * h * h - 6 * h * (hx[3] - x) + 9 * (hx[3] - x) * (hx[3] - x));
  else /* if (x > hx[3]) && (x <= hx[4]) */
...
double xfi(double a, double b, int n, int i, FILE *out) {
  ...
  for (j = 0; j < 5; j++) fprintf(out, " %d", hi[j]);
  fprintf(out, "] hx=[");
  for (j = 0; j < 5; j++) fprintf(out, " %g", hx[j]);
  fprintf(out, "]\n");
}
...
#define TESTBASE 500
  {
    ...
    for (j = 0; j < nb; j++) xfi(a, b, nb, j, tst);
    ...
    for (i = 0; i < nb; i++)
      for (k = 0; k < pts->n; k++)
        add_to_entry_matrix(eqs, j, i,
                            fi(a, b, nb, i, x[k]) * fi(a, b, nb, j, x[k]));
  ...
  if (alloc_spl(spl, nb) == 0) {
    for (i = 0; i < spl->n; i++) {
      ...
      xx += 10.0 * DBL_EPSILON;  // zabezpieczenie przed ulokowaniem punktu w
